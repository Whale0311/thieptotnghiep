...
export default function App() {
  ...
  const baseRSVP = useMemo(
    ...
    [ceremonyReply, draft.guestName],
  )

  function startRSVP(attending: boolean) {
    setDraft({ guestName: '', graduationAttendance: attending })
    setSubmissionError('')
    setSubmissionState('idle')
    setStep('name')
  }

  function saveName(guestName: string) {
    setDraft((current) => ({ ...current, guestName }))
    setStep('party')
  }
  ...
  function chooseParty(attending: boolean) {
    ...
    if (attending) {
      if (!partyOptionsAvailable) {
        setSubmissionState('error')
        setSubmissionError('Hiện chưa có thời gian tiệc phù hợp để lựa chọn. Vui lòng liên hệ chủ tiệc.')
        return
      }
    ...
    <main className="site-shell">
          ...
          <section className="hero reveal">
            ...
            <div className={`photo-frame ${!eventConfig.showGraduatePhoto ? 'photo-frame--hidden' : ''}`}>
              {eventConfig.showGraduatePhoto ? (
                <img src={eventConfig.graduatePhoto} alt={`Chân dung ${eventConfig.graduateName}`} />
              ) : (
                <span aria-hidden="true">✦</span>
              )}
            </div>
            <p className="invitation-copy">{eventConfig.invitationMessage}</p>
            <div className="hero__rule" aria-hidden="true"><span>✦</span></div>
          </section>

          <div className="content-column">
            <SectionCard eyebrow="Thông tin buổi lễ" title="Hẹn gặp bạn vào ngày đặc biệt này">
              <div className="event-info">
                <div className="event-info__icon" aria-hidden="true">◷</div>
                <div><span>Thời gian</span><strong>{eventConfig.ceremonyTime} — {eventConfig.ceremonyDate}</strong></div>
                <div className="event-info__icon" aria-hidden="true">⌖</div>
                <div>
                  <span>Địa điểm</span><strong>{eventConfig.ceremonyLocation}</strong>
                  {eventConfig.ceremonyAddress && <small>{eventConfig.ceremonyAddress}</small>}
                  {eventConfig.mapUrl && <a href={eventConfig.mapUrl} target="_blank" rel="noreferrer">Mở bản đồ ↗</a>}
                </div>
              </div>
            </SectionCard>

            <SectionCard eyebrow="Xác nhận tham dự" title="Bạn có thể đến chung vui cùng mình chứ?">
              <p className="section-copy">Chỉ mất một phút để mình chuẩn bị chu đáo hơn cho ngày gặp mặt.</p>
              <AttendanceChoice onChoose={startRSVP} />
            </SectionCard>

            <aside className="contact-card">
              <span aria-hidden="true">✦</span>
              <div><small>Cần hỗ trợ?</small><strong>{eventConfig.contactName}</strong></div>
              <a href={contactHref}>{eventConfig.contactPhone}</a>
              {eventConfig.contactZalo && <a href={eventConfig.contactZalo} target="_blank" rel="noreferrer">Nhắn Zalo ↗</a>}
            </aside>
          </div>
        </>
      )}
      ...
      {step === 'name' && ceremonyReply !== null && (
        <div className="flow-column">
          <SectionCard eyebrow="Một chút thông tin" title="Mình nên gọi bạn là gì?">
            <GuestNameForm
      ...
      {step === 'party' && (
        <div className="flow-column">
          <SectionCard eyebrow="🥂 Dự định tiệc tốt nghiệp" title="Bạn có thể tham gia bữa tiệc nhỏ cùng mình không?">
            ...
            {submissionState === 'submitting' && <p className="submission-message" aria-live="polite">Đang gửi xác nhận...</p>}
            {submissionError && (
              <div className="submission-error" role="alert">
                <p>{submissionError}</p>
                <button className="text-button" onClick={() => chooseParty(false)} disabled={submissionState === 'submitting'}>Thử lại</button>
              </div>
            )}
          </SectionCard>
        </div>
      ...
      {step === 'party-details' && (
        <div className="flow-column">
          <SectionCard eyebrow="🥂 Dự định tiệc tốt nghiệp" title="Khung thời gian nào phù hợp với bạn?">
            <PartyDetails
              isSubmitting={submissionState === 'submitting'}
              onSubmit={(details) => void submitRSVP({ ...baseRSVP, partyAttendance: true, ...details })}
            />
      ...
      {step === 'thanks' && completedRSVP && <ThankYou rsvp={completedRSVP} />}
