...
import { cleanText, getRSVPStatus, statusLabels, validateRSVP } from '../utils/rsvp'
...
export async function sendRSVPToDiscord(rsvp: RSVPData): Promise<void> {
  ...
  if (!eventConfig.discordWebhookUrl) {
    throw new Error('Kênh nhận phản hồi chưa được cấu hình. Vui lòng liên hệ chủ tiệc.')
  }
  ...
  const fields = [
    ...
    { name: '🥂 Tiệc tốt nghiệp', value: attendanceLabel(rsvp.partyAttendance), inline: true },
  ]

  if (rsvp.partyAttendance) {
    fields.push(
      { name: '📅 Thời gian tiệc', value: cleanText(rsvp.partyDateTime!, 120), inline: true },
      { name: '👥 Số người', value: String(rsvp.numberOfAttendees), inline: true },
      {
        name: '🍽️ Lưu ý món ăn',
  ...
  fields.push(
    ...
    { name: '🕐 Thời gian phản hồi', value: formatSubmittedAt(rsvp.submittedAt), inline: true },
  )
